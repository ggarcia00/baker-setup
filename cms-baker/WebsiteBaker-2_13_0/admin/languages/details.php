...
//if (!function_exists('make_dir')){require(WB_PATH.'/framework/functions.php');}
    ...
    $admin = new admin('Addons', $sAddonType.'s_view');
...
// Get addon directory
    ...
    $sAddonFile = $sAddonDir.'.php';
// Check if the add-on exists
    if (!file_exists(WB_PATH.$sAddonAppDir.$sAddonFile)) {
...
// check if a addon description exists for the displayed backend language
    ...
    if (function_exists('file_get_contents') && file_exists(WB_PATH.$sAddonAppDir.'/languages/'.LANGUAGE .'.php')) {
        // read contents of the addon language file into string
        $data = @file_get_contents(WB_PATH .$sAddonAppDir .'/languages/' .LANGUAGE .'.php');
