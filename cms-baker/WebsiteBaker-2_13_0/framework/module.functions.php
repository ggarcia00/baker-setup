...
// this function checks the validity of the specified module directory
    if(!is_callable('check_module_dir')) {
       function check_module_dir($sAddonName) {
          ...
          return (is_readable(WB_PATH .'/modules/' .$sAddonName .'/info.php')) ? $sAddonName : '';
       }
    }
...
// this function displays the "Edit CSS" button in modify.php
    if (!is_callable('edit_module_css')) {
       function edit_module_css($sAddonName) {
          ...
          if (!is_readable(WB_PATH.'/modules/edit_module_files.php')){ return;}
          // check if specified module directory is valid
          if ($sAddonPath = check_module_dir($sAddonName) == ''){ return;}
          ...
          // check if frontend.css or backend.css exist
            $frontend_css = (is_readable($sAddonPath.'/'.$aCssFiles['2']) ? $aCssFiles['2'] : 'frontend.css');
            $backend_css  = (is_readable($sAddonPath.'/'.$aCssFiles['3']) ? $aCssFiles['3'] : 'backend.css');
...
?>
             <form id="edit_module_file" action="<?php echo WB_URL .'/modules/edit_module_files.php';?>"
...
// this function displays a button to toggle between CSS files (invoked from edit_css.php)
    if (!is_callable('toggle_css_file')) {
       function toggle_css_file($sAddonName, $base_css_file = 'frontend.css') {
          global $page_id, $section_id, $admin;
          $aCssFiles = ['frontend.css', 'backend.css','frontendUser.css', 'backendUser.css'];
          // check if the required edit_module_css.php file exists
          if (!is_readable(WB_PATH .'/modules/edit_module_files.php')){ return;}
          ...
          if (!in_array($base_css_file, $aCssFiles)){ return;}
          // display button to toggle between the two CSS files: frontend.css, backend.css
          $sUserFile = substr($base_css_file, -8,4);
          $base_css_file   = ($sUserFile=='User' ? str_replace($sUserFile, '',$base_css_file) : $base_css_file);
          $toggle_file = ($base_css_file == 'frontend.css') ? 'backend.css' : 'frontend.css';

          if (mod_file_exists($sAddonName, $toggle_file)) {
             ?>
             <form id="toggle_module_file" action="<?php echo WB_URL .'/modules/edit_module_files.php';?>" method="post" style="margin: 0; text-align:right;">
                <?php echo $admin->getFTAN();?>
                <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
                <input type="hidden" name="section_id" value="<?php echo $section_id; ?>" />
                <input type="hidden" name="mod_dir" value="<?php echo $sAddonName; ?>" />
...
// function to check if the optional module Javascript files are loaded into the <head> section
    if (!is_callable('requires_module_js')) {
       function requires_module_js($mymod_dir, $js_file) {
          $aJsFiles = ['frontend.js', 'backend.js','frontendUser.js', 'backendUser.js'];
