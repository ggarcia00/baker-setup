...
CKEDITOR.dialog.add( 'wblink', function( editor ) {
    ...
    var addonRegex = /\[(wblink([\d]+))\??addon\=([^&]*)\&?item\=([^\]]*)[^\]]*\]/,
        pageRegex = /\[wblink(\d*)[^\]]*\]/,
        wblinkRegex = /\[wblink(\d*)\]/;

    var popupRegex = /\s*window.open\(\s*this\.href\s*,\s*(?:'([^']*)'|null)\s*,\s*'([^']*)'\s*\)\s*;\s*return\s*false;*\s*/;
    var popupFeaturesRegex = /(?:^|,)([^=]+)=(\d+|yes|no)/gi;

/**
 * global vars for pages and addon Array
 *
 * @global pageIndex,    hold the selected pageItemSelectBox id
 * @global urlLinkType,  hold the selected linktype
 * @global urlLinkMatch, hold the stored data.url.url
 *
 */
    var pageIndex=null,
        iPageId=null,
    ...
    var parseLink = function( editor, element ) {
            ...
            if ( !retval.type ) {
                ...
                // urlRegex matches empty strings, so need to check for href as well.
                    ...
                    urlLinkMatch = retval.url.url;
                } else
                    retval.type = 'url';
                urlLinkType  = retval.type;
            ...
            if ( element ) {
                ...
                CKEDITOR.plugins.wblink.synAnchorSelector ? retval.adv.advCSSClasses = getLinkClass( element ) : advAttr( 'advCSSClasses', 'class' );
            ...
            if ( CKEDITOR.plugins.wblink.fakeAnchor ) {
                ...
                for ( i = 0, count = imgs.count(); i < count; i++ ) {
                    if ( ( item = CKEDITOR.plugins.wblink.tryRestoreFakeAnchor( editor, imgs.getItem( i ) ) ) )
    ...
    // Handles the event when the "addonlink" selection input is changed.
//    var parseAddonLink = function() {
//            if ( PagesItemSelectBox && AddonItemsSelectBox ) {
//                dialog   = this.getDialog();
//                element = dialog.getContentElement( 'info', 'addlink' );
//                selectedPage = this.getInputElement().$;
            ...
            elements: [
                ...
                children: [
                    ...
                    children: [
                        ...
                        items: PagesItemSelectBox,
                        onChange : function() {
//  $pattern = '/\[(wblink([\d]+))\??addon\=([^&]*)\&?item\=([^\]]*)[^\]]*\]/i';
//    Array (
//        [0] => [wblink158?addon=topics&item=25]
//        [1] => wblink158
//        [2] => 158
//        [3] => topics
//        [4] => 25
//    )
                            if ( PagesItemSelectBox ) { //  && (urlLinkType == 'wblink')
                                selPage  = this.getValue();
                                dialog   = this.getDialog();
                                pageId = selPage.match(wblinkRegex);
                                element = dialog.getContentElement( 'info', 'addlink' );
                                selectedPage = this.getInputElement().$;
                                var detailAddonLink = '',
                                    selectedAddon = "",
                                    setAddon = false;
                                i=0; iPageId=0;   //p=pageId[1],
                                for( i in PagesItemSelectBox ){
                                  if( selPage == PagesItemSelectBox[i][1] ){
                                    iPageId = ( PagesItemSelectBox[i][4] ) ;
                                    selectedAddon = ( PagesItemSelectBox[i][3] ) ;
                                        if( selectedAddon != '' ) {
                                            pageIndex=i;
                                       }
                                    }
                                }
                                seladdon = dialog.getContentElement( 'info', 'addonlink');
                                htmlOptionsList = seladdon.getInputElement( 'info', 'addonlink').$;
                                if ( iPageId && AddonItemsSelectBox && ( AddonItemsSelectBox[iPageId] ) ) {
                                    seladdon.clear();
                                    seladdon.add( linkLang.selectAddon, '' );
                                    if ( pageIndex )
                                        detailAddonLink = PagesItemSelectBox[pageIndex].slice(3,4);
                                    s=0;i=0;
//                                    sel.add(eval('linkLang.select' + ModuleList[pageId[1]]), "");
                                    for (s = 0; s < AddonItemsSelectBox[iPageId].length; s++) {
                                        for (i = 0; i < AddonItemsSelectBox[iPageId][s].length; i++) {
                                            seladdon.add( AddonItemsSelectBox[iPageId][s][i][0], AddonItemsSelectBox[iPageId][s][i][1] );
//                                            seladdon.add( selectValue );
                                            if( detailAddonLink == AddonItemsSelectBox[iPageId][s][i][1] ){
//                                                seladdon.selected = true;
                                            }
                                        }
                                    }
/*
//                                    seladdon.setValue(selectValue);
//                                    }
*/
                                    seladdon.getElement().show();
                                }
                                else
                                    seladdon.getElement().hide();
                            } // urlLinkType
                        },
...
//                        },
                        setup: function( data ) {
                          // first entry
                            selPage = PagesItemSelectBox[0].slice(1,2);
                          // fetch saved wblink or addonlink
                            if (data.url) {
                                wbl = data.url.url;
                                tmpId = wbl.match(addonRegex);
                                i=0;
                                // is it a addonlink, then convert to wblink
                                if( tmpId ) wbl = '[' + tmpId[1] + ']';
                                for(i in PagesItemSelectBox){
                                    if(wbl == PagesItemSelectBox[i].slice(1,2)){
                                        pageIndex = i;
                                        if(tmpId) {
                                            // replace the empty index with addonlink
                                            PagesItemSelectBox[i].splice(3,1);
                                            PagesItemSelectBox[i].splice(3,0,data.url.url);
                                        } else {
                                            // remove addonlink from page item
                                            PagesItemSelectBox[i].splice(3,1);
                                            PagesItemSelectBox[i].splice(3,0,"");
                                        }
                                        selPage = PagesItemSelectBox[i].slice(1,2);
                                    }
                                }
                            }
...

                {
                    ...
                    children: [
                        ...
                        id: 'addonlink',
                        type: 'select',
                        label: linkLang.addonItem,
                        items: AddonItemsSelectBox,
//                        items: new Array(new Array()),
                        onChange : function() {
                            if (PagesItemSelectBox) {
                                dialog   = this.getDialog();
                                seladdon = dialog.getContentElement( 'info', 'addonlink');
                                htmlOptionsList = seladdon.getInputElement( 'info', 'addonlink').$;
                            }
...
//        [4] => 25
//    )
                            if ( AddonItemsSelectBox && (urlLinkType == 'wblink') ) { //
                                if (data.url) {
                                    wblinkUrl = data.url.url;
                                    tmpId = wblinkUrl.match(addonRegex);
                                    isAddonUrl = true;
                                    if( tmpId==null ) {
                                        tmpId = wblinkUrl.match(pageRegex);
                                        isAddonUrl = false;
                                    }
                                    iPageId=0;
                                    if( isAddonUrl )
                                        ...
                                        iPageId=tmpId[1];
                                    // get the html optionsList to set selected=true
                                    htmlOptionsList = seladdon.getInputElement( 'info', 'addonlink').$;
                                    if ( AddonItemsSelectBox[iPageId] ) {
                                        s=0;i=0;
                                        seladdon.clear();
                                        seladdon.add( linkLang.selectAddon, '' );
                                        for (s = 0; s < AddonItemsSelectBox[iPageId].length; s++) {
                                        ...
                                        item=0;
                                        for (item = 0; item < htmlOptionsList.length; item++) {
                                            if( wblinkUrl == htmlOptionsList[item].value ) {
                                                htmlOptionsList[item].selected = true
                                            }
                                        }
                                        seladdon.getElement().show();
                                    } else {
...
//                        },
                        commit: function( data ) {
                            if ( !data.addonlink )
                                data.addonlink = {};
                            data.addonlink.url = this.getValue();
                        }
                    }
                    ],
                    setup: function( data ) {
        ...
        onOk: function() {
            ...
            switch ( data.type || 'url' ) {
                case 'url':
                    ...
                    break;
                case 'wblink':
                  if (data.addonlink && data.addonlink.url !== "") {
                    var url = ( CKEDITOR.tools.trim( data.addonlink.url ) );
                  } else {
            ...
            if ( !this._.selectedElement ) {
                ...
                if ( data.adv && data.adv.advName && CKEDITOR.plugins.wblink.synAnchorSelector )
