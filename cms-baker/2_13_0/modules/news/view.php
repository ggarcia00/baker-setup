...
//    $sTimeFormat = PreCheck::dateFormatToStrftime($sTimeFormat);
    $addBracket = (function (){
        $aList = func_get_args();
        ...
        while (($post = $query_posts->fetchRow( MYSQLI_ASSOC )))
        {
            ...
            if (
                ...
                if ($post['published_when'] > $post['posted_when']) {
                    $post_date = strftime ($sDateFormat, $post['published_when']+TIMEZONE);
                    $post_time = date ($sTimeFormat, $post['published_when']+TIMEZONE);
                } else {
                    $post_date = strftime ($sDateFormat, $post['posted_when']+TIMEZONE);
                    $post_time = date ($sTimeFormat, $post['posted_when']+TIMEZONE);
                }
                $publ_date     = strftime ($sDateFormat,$post['published_when']+TIMEZONE);
                $publ_time     = date ($sTimeFormat,$post['published_when']+TIMEZONE);
                $modified_date = strftime ($sDateFormat,$post['modified_when']+TIMEZONE);
                $modified_time = date ($sTimeFormat,$post['modified_when']+TIMEZONE);
...
//                $oApp->
                $create_date    = strftime ($sDateFormat, $post['created_when']+TIMEZONE);
                $create_time    = date ($sTimeFormat, $post['created_when']+TIMEZONE);
...
//            $bShortlink       = is_readable(WB_PATH.'/short.php');
            ...
            if (($query_comments = $database->query($aSql[9]))) {
                while (($comment = $query_comments->fetchRow( MYSQLI_ASSOC ))) {
                    ...
                    // Print comments loop
                    $commented_date = strftime($sDateFormat, $comment['commented_when']+TIMEZONE);
                    $commented_time = date($sTimeFormat, $comment['commented_when']+TIMEZONE);
